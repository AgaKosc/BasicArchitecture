...
class DataGenerator:
        ...
        if case == 1:
            ...
            return 'chars', \
                   rstr.rstr(string.ascii_letters + " '-" + string.digits, 1, maxLength,
                             include=random.choice(improperCharacters)).strip()
        else:
            return 'length', \
    ...
    def generateProperEmail():
        ...
        while True:
            email = '{0}{1}@{2}.{3}'.format(random.choice(string.ascii_letters),
    ...
    def generateWrongEmail():
        ...
        case = random.randint(1, 2)
        if case == 1:
            return '{0}@{1}.{2}'.format(rstr.rstr(string.printable, include=random.choice(improperCharacters),
                                                  exclude='\n\t\r"'),
                                        rstr.rstr(string.printable, include=random.choice(improperCharacters),
                                                  exclude='\n\t\r"'),
                                        random.choice(['pl', 'com', 'net', 'uk', 'gov', 'us']))
        else:
    ...
    def generateWrongPhoneNumber(self):
        ...
        if case == 1:
            ...
            return rstr.rstr(string.printable, include=random.choice(string.ascii_letters), exclude='\n\t\r"')
